...
void * thread_fun(void *arg){
  ...
  while(1){
      if(cur>=size)break;
      if((acc[tra[cur].ac1-1001].locked==0)){
        if(tra[cur].ac2!=0){
          if(acc[tra[cur].ac2-1001].locked!=0)continue;
        }
        ...
        if((tra[cur].ac2!=0)){
          if(acc[tra[cur].ac2-1001].locked!=0){
            pthread_mutex_unlock(&lock);
            continue;
          }
        }
        acc[tra[cur].ac1-1001].locked=1;
        if(tra[cur].ac2!=0){
          acc[tra[cur].ac2-1001].locked=1;
        }
        ac1=tra[cur].ac1;
        ac2=tra[cur].ac2;
        ...
        pthread_mutex_unlock(&lock);
        if(type==1){
          acc[(ac1-1001)].balance+=(amount-(0.010*amount));
          acc[ac1-1001].locked=0;
        }
        if(type==2){
          acc[(ac1-1001)].balance-=(amount+(0.010*amount));
          acc[ac1-1001].locked=0;
        ...
        if(type==3){
          acc[ac1-1001].balance+=(0.0710*(acc[ac1-1001].balance));
          acc[ac1-1001].locked=0;
        }
        if(type==4){
          acc[(ac1-1001)].balance-=(amount+(0.010*amount));
          acc[(ac2-1001)].balance+=(amount-(0.010*amount)); 
          acc[ac2-1001].locked=0;
        }
        acc[ac1-1001].locked=0;
        if(ac2!=0) acc[ac2-1001].locked=0;
        if(cur>=size)break;
      }
  }
}
